...
for count,server in enumerate(serverlist):
    if server['capable'] == 'false':
        ...
        continue
    if server['energy'] == 'true':
        print("-->Server %d %-20s capable of collecting Energy stats and enabled"%(count,server['name']))
        continue
    print("-->Server %d %-20s capable of collecting Energy stats but need enabling now"%(count,server['name']))
    hmc.set_energy_flags(server['name'],'true')
